...
async function sortByLevel () {
  ...
  if (sortingASC.level) { courses.sort((a, b) => lvlMap[a.level] - lvlMap[b.level]) } else { courses.sort((a, b) => lvlMap[b.level] - lvlMap[a.level]) }
...
function toggleSortingOptions () {
  ...
  if (hidden) {
    for (const option of options) { option.classList.remove('inactive') }
  } else {
    for (const option of options) { option.classList.add('inactive') }
  }
}
...
function toggleOptionCheckboxesVisibility (e) {
  if (document.querySelector('.filtering-option .options').classList.contains('inactive')) { document.querySelector('.filtering-option .options').classList.remove('inactive') } else document.querySelector('.filtering-option .options').classList.add('inactive')
}
